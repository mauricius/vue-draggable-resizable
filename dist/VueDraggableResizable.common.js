...
function snapToGrid(grid, pendingX, pendingY) {
  ...
  var _ref = typeof scale === 'number' ? [scale, scale] : scale,
...
/* harmony default export */ var vue_draggable_resizablevue_type_script_lang_js_ = ({
  ...
  mounted: function mounted() {
    ...
    var _getComputedSize = getComputedSize(this.$el),
        _getComputedSize2 = _slicedToArray(_getComputedSize, 2),
        width = _getComputedSize2[0],
        height = _getComputedSize2[1];
    ...
    if ('ResizeObserver' in window) {
      this.parentResizeObserver = new ResizeObserver(function () {
        _this.checkParentSize();

        _this.handleParentResize();
      });
  ...
  methods: {
    ...
    moveHorizontally: function moveHorizontally(val) {
      ...
      var _snapToGrid3 = snapToGrid(this.grid, val, this.top, 1),
    ...
    moveVertically: function moveVertically(val) {
      // should calculate with scale 1.
      var _snapToGrid5 = snapToGrid(this.grid, this.left, val, 1),
    ...
    handleParentResize: function handleParentResize() {
      var grid = this.grid;
      this.bounds = this.calcDragLimits();
      var left = restrictToBounds(this.left, this.bounds.minLeft, this.bounds.maxLeft);
      var top = restrictToBounds(this.top, this.bounds.minTop, this.bounds.maxTop);
      var right = restrictToBounds(this.right, this.bounds.minRight, this.bounds.maxRight);
      var bottom = restrictToBounds(this.bottom, this.bounds.minBottom, this.bounds.maxBottom);
      this.left = left;
      this.top = top;
      this.right = right;
      this.bottom = bottom;
    ...
    handleResize: function handleResize(e) {
      ...
      var _snapToGrid7 = snapToGrid(this.grid, tmpDeltaX, tmpDeltaY, this.scale),
          _snapToGrid8 = _slicedToArray(_snapToGrid7, 2),
          deltaX = _snapToGrid8[0],
          deltaY = _snapToGrid8[1];
    ...
    changeWidth: function changeWidth(val) {
      // should calculate with scale 1.
      var _snapToGrid9 = snapToGrid(this.grid, val, 0, 1),
          _snapToGrid10 = _slicedToArray(_snapToGrid9, 2),
          newWidth = _snapToGrid10[0],
          _ = _snapToGrid10[1];
    ...
    changeHeight: function changeHeight(val) {
      // should calculate with scale 1.
      var _snapToGrid11 = snapToGrid(this.grid, 0, val, 1),
          _snapToGrid12 = _slicedToArray(_snapToGrid11, 2),
          _ = _snapToGrid12[0],
          newHeight = _snapToGrid12[1];
...
function install(Vue) {
  ...
  Vue.component('VueDraggableResizable', _components_vue_draggable_resizable__WEBPACK_IMPORTED_MODULE_1__[/* default */ "a"]);
}
var plugin = {
  install: install
};
...
/******/ })["default"];
//# sourceMappingURL=VueDraggableResizable.common.js.map